...
const addOperation = () => {
  let operation = {};
  console.log(formNewOperation, '0aqui');
    ...
    if (operations.length > 0) {
      ...
      operations.forEach((operation) => {
        ...
        if (operation.type === 'Gasto') {
            operation.monto = `-$${operation.monto}`
            colorMonto = '#E10000';
            totalResta = totalResta - operation.monto
            gasto.innerHTML = `${totalResta}`
        }else{
            operation.monto = `+$${operation.monto}`
            colorMonto = '#00D900'
            totalSuma = totalSuma + operation.monto
            gasto.innerHTML = `${totalSuma}`
        }
        const totalFinal = totalSuma + totalResta;
        total.innerHTML = `${totalFinal}`
        let node = document.createElement("div");
        node.innerHTML = `<div class="columns has-text-weight-medium is-mobile">
        <div class="column is-3">${operation.description}</div>
        <div class="column is-2">
          <span class="tag is-info is-light is-medium"
            >${operation.category}</span
          >
        </div>
        <div class="column is-3">${operation.date}</div>
        <div class="column is-2" style= "color: ${colorMonto}; font-weight: bold;">${operation.monto}</div>
        <div class="column is-2">
